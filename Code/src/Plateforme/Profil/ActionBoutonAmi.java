 ...
 * @author theodore
 */
public class ActionBoutonAmi implements EventHandler<ActionEvent> {
    /**
     * le profil auquel est associé l'ActionBoutonProfil
     */    
    private ProfilAmis profil;

    public ActionBoutonAmi(ProfilAmis profil){
        this.profil = profil;
    }
    /**
    ...
    public void handle(ActionEvent a){
        ...
        if(b.getUserData().equals("search")) {
            ...
            List<Jeu> v;
            try {
                v = this.profil.getPlateforme().getConnexion().getJeux();
                Collections.sort(v);
                ListeStat nLs = new ListeStat(this.profil.getPlateforme(), v, this.profil.getListeStat().getListePartie());
                this.profil.setListeStat(nLs);
                this.profil.maj();
